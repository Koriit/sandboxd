...
The network-control subsystem uses per-component liveness probes for health monitoring (see [networking-design.md](networking-design.md), § Health monitoring). These probes verify that individual components (nftables rules, local DNS resolver, Envoy, mitmproxy, sandbox daemon) are alive but do not guarantee that traffic is actually being proxied through the full pipeline.
...
A dedicated container runs a minimal HTTPS server with a single endpoint. It runs on the host side — the same Docker daemon (Linux) or Colima instance (macOS) as the gateway container — and is attached to the per-session network so it is reachable through the gateway pipeline. Its sole purpose is to serve as the target for end-to-end health probes.
...
1. A probe process runs inside the sandbox VM (not inside any container within the VM)
2. The probe makes an HTTPS request to the health-check container
3. The request exits the VM through the full gateway pipeline: nftables PREROUTING DNAT → Envoy → mitmproxy → health-check container
...
The health-check endpoint must be reachable from inside the sandbox VM (to exercise the real data path), which means sandboxed code can also reach it. A separate container with blocked outbound traffic ensures that even if the endpoint is compromised, the attacker gains a container that is **more locked down** than the sandbox VM they are already in — no outbound connectivity, no secrets, no control plane access.
...
* probe process management in the VM
