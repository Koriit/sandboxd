...
fn integration_lite_image_rebuild_fresh_tag_produces_image() {
    ...
    rebuild_lite_image(&version, false, std::env::temp_dir().as_path()).expect("rebuild_lite_image must succeed on fresh tag");
...
fn integration_lite_image_rebuild_after_ensure_image_succeeds() {
    ...
    let first = ensure_image(&version, std::env::temp_dir().as_path()).expect("ensure_image first call");
    assert!(
        matches!(first, EnsureImageOutcome::Built { .. }),
        "ensure_image first call must build, got {first:?}",
    );

    let second = ensure_image(&version, std::env::temp_dir().as_path()).expect("ensure_image second call");
    assert_eq!(
        second,
    ...
    rebuild_lite_image(&version, false, std::env::temp_dir().as_path())
...
fn integration_lite_image_rebuild_with_no_cache_succeeds() {
    ...
    rebuild_lite_image(&version, true, std::env::temp_dir().as_path()).expect("rebuild_lite_image with no_cache=true must succeed");
