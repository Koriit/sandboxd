...
impl LimaManager {
    ...
    fn relax_lima_instance_perms(&self, vm: &str) {
        #[cfg(unix)]
        {
            ...
            // Lima's shared SSH identity at `_config/user` is generated by
            // limactl-as-daemon at mode 0600 and lives outside any per-VM
            // instance dir. Under the spawn-helper exec path the per-VM
            // hostagent runs as the operator uid and SSHes into the guest
            // using this same identity file. Widen to 0640 so the operator
            // (a member of the `sandbox` group via `setup-dev-env`) can
            // read it. The matching `_config/user.pub` and
            // `_config/networks.yaml` are already world-readable; we widen
            // all `_config/` files defensively so a future Lima upgrade
            // that adds a new 0600 file there does not silently regress
            // the cross-user SSH probe.
            let config_dir = lima_home.join("_config");
                ...
                match std::fs::read_dir(&config_dir) {
                    Err(e) => {
                        warn!(
                            ...
                            error = %e,
                            "failed to read Lima _config dir for permission relaxation"
                        );
                    }
                    Ok(entries) => {
                        for entry in entries.flatten() {
                            ...
                            if path.is_file() {
                                if let Err(e) = std::fs::set_permissions(
                                    ...
                                    warn!(
                                        ...
                                        path = %path.display(),
                                        error = %e,
                                        "failed to chmod Lima _config file to 0640"
                                    );
                                } else {
                                    debug!(
                                        vm,
                                        path = %path.display(),
                                        "chmod Lima _config file to 0640"
                                    );
                                }
                            }
                        }
