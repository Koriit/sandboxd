...
dynamic_resources:
  lds_config:
    ...
    path_config_source:
      path: {LISTENER_FILE_IN_CONTAINER}
      watched_directory:
        path: {LISTENER_DIR_IN_CONTAINER}
            ...
            r#"            access_log:
              - name: envoy.access_loggers.file
                typed_config:
                  ...
                  path: {ENVOY_ACCESS_LOG_IN_CONTAINER}
                  log_format:
                    json_format:
...
#
# Served via filesystem LDS from `{LISTENER_FILE_IN_CONTAINER}`. sandboxd rewrites
