...
The suite runs the daemon as the unprivileged `sandbox` system user via `sudo -u sandbox`, while the test runner acts as the *operator* (a separate uid) — the real cross-user path. `make test-e2e` is the single env-agnostic trigger. All sudo is pre-authorized by `make setup-dev-env`, which installs a NOPASSWD sudoers fragment (`/etc/sudoers.d/sandboxd-test`) granting the operator blanket passwordless impersonation of the unprivileged `sandbox` user, and creates `/var/lib/sandbox-test` (the test daemon's base dir and socket parent) with `sandbox:sandbox 0750`. The test base dir is **deliberately separate** from the production `/var/lib/sandbox` (which `install.sh` owns) so a real prod install and the harness can coexist on one host — the harness's state-dir reset can never wipe a prod daemon's `sessions.db`/`.install-state.json`, and the two never share one `sessions.db`. Runtime sudo is exclusively `sudo -u sandbox` — no root, no systemd. When running or debugging:
...
- **Socket path:** `/var/lib/sandbox-test/sandboxd.sock` (inside the test base dir; daemon creates it with no elevated privileges). Note: a *production* install's daemon uses `/run/sandbox/sandboxd.sock` with base dir `/var/lib/sandbox` — separate from the harness on both axes.
- **The test-cap'd `sandbox-lima-helper` is reinstalled on every run.** `make install-lima-helper-test-cap` (and `make setup-dev-env`) routes through a `.PHONY` cargo-build target and reinstalls + `setcap`s unconditionally (no `cmp` short-circuit — the recipe documents why), so any helper source change is always picked up. No manual stamp deletion is needed after a helper change.
