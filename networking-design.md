...
**The VM has no usable bypass path.** The VM has **two** host-provided NICs: the per-session network NIC (Docker bridge on Linux / socket_vmnet on macOS), whose segment routes out only via the gateway; and Lima's **mandatory management NIC** (a QEMU user-mode/slirp `eth0`, used for SSH/readiness). The no-bypass guarantee therefore does **not** rest on "only one NIC exists" — the slirp NIC is present and NATs to the host by default. It rests on two enforced properties: **(a)** the per-session NIC's segment has no route out except the gateway, and **(b)** the management NIC is **`-netdev user,restrict=on`** (libslirp drops all guest-initiated egress while preserving host→guest SSH `hostfwd` — see "Management slirp NIC must be `restrict=on`"). With both in place, a root guest rewriting its routing/firewall still cannot egress except through the gateway. (A root guest cannot fabricate a *new* host-backed NIC; it can only abuse one QEMU already gave it — which is why (b) closes the slirp NIC. On **Linux** this is implemented in the `lima.rs` qemu-wrapper — [#65](https://github.com/Koriit/sandboxd/issues/65), commit `4b134ec`; macOS mirrors it.) The only other communication path is vsock, which is a host-guest socket family (AF_VSOCK) that does not carry IP traffic and does not traverse the proxy pipeline. See [sandbox-design.md § Control channel (vsock)](sandbox-design.md#control-channel-vsock).
...
* **One *egress-capable* NIC, IPv4 only.** The VM's gateway-facing NIC — a virtio-net device on the per-session network (Docker bridge on Linux, socket_vmnet on macOS) — is the only path to the internet, IPv4-only (no IPv6 assigned). The VM also has Lima's **mandatory management NIC** (slirp `eth0`), which exists for SSH/readiness but is **`-netdev user,restrict=on`** so it carries no guest egress (see "Management slirp NIC must be `restrict=on`"). No *other* interfaces exist (no extra NICs, no virtio-fs, no USB network adapters). Earlier text claiming "exactly one NIC, no others" was inaccurate — the management NIC is always present; the guarantee is that it cannot egress, not that it is absent.
* **Default route to gateway.** The default route points to the gateway container's IP on the per-session subnet. All traffic (except loopback and vsock) exits via this route.
* **Static or DHCP.** The VM's IP on the per-session subnet is assigned during provisioning — either statically (Lima template / cloud-init) or via DHCP. On **macOS** the socket_vmnet segment runs in network-identifier mode with **no DHCP**, so the VM's IP and its default route (→ the gateway container) are assigned **statically** via cloud-init.
* **No alternate routes.** No other routes exist. The routing table contains only the default route to the gateway, the connected per-session subnet route, and loopback.

#### DNS configuration

`/etc/resolv.conf` points to the gateway container's DNS resolver IP on the per-session subnet. This makes the gateway's policy-aware DNS resolver the default resolver for all applications in the VM.
...
The inner Docker daemon's NAT translates container source IPs to the VM's IP on the per-session subnet. The gateway's proxy pipeline sees the VM's IP as the source — it does not see or care about inner container IPs. This is transparent and requires no special configuration.
...
The connectivity mechanism differs by platform, but both achieve the same result: the VM's gateway-facing NIC has its default route pointing at the gateway container's IP, and traffic arrives at the gateway with original destination intact so that PREROUTING DNAT works correctly. (The VM's other NIC, Lima's management slirp `eth0`, is `restrict=on` and carries no egress.)
...
On macOS, Docker does not run natively — it runs inside a Linux VM. A sandbox VM therefore cannot attach to a Docker bridge on the macOS host, because that bridge exists inside another VM's network namespace.

The solution mirrors Linux's per-session Docker-bridge isolation using a per-session **socket_vmnet** pool plus a sandboxd-managed Lima gateway instance:

1. **socket_vmnet pool** — sandboxd runs N root `socket_vmnet` daemons (one per pool slot), each in **host mode with a unique `--vmnet-network-identifier`** so the kernel places it on its own **isolated** L2 network (no cross-slot leakage) with **no DHCP** — addresses are assigned statically. Each slot gets a **/29** subnet. The L3 participants are the gateway container (`B+3`) and the sandbox VM (`B+4`); the gateway Lima VM's socket_vmnet NIC (`B+2`, the macvlan parent) is link-only (a macvlan parent needs no address) and an isolated network-identifier segment has no host-side gateway. /29 is retained for headroom — a /30's 2 usable IPs are tight, and whether the host remains on the segment is pending the isolation spike (see the macOS-support spec § Subnet allocation / task A). The pool size is configurable (`max_macos_sessions`, default 8). The per-slot network-identifier is what isolates each segment — the same isolation property as Linux's per-session Docker bridges.

   The pool is **pre-provisioned** *not* as a startup-cost optimization (VM boot dwarfs socket_vmnet init) but because **Lima attaches a VM's NICs only at instance-creation time and offers no runtime NIC-attach**: the gateway Lima instance must carry all N socket_vmnet NICs from the moment it boots, since adding one later would require recreating it and disrupting every live session.

2. **sandboxd-managed Lima gateway instance (`sandboxd-gateway`)** — sandboxd manages a single Lima instance (vmType `qemu`, HVF-accelerated) running Docker, hosting all gateway containers across all sessions. It is **not Colima** — sandboxd drives Lima directly. It is independent of whatever Docker the developer uses (Docker Desktop, Colima, OrbStack); the developer never interacts with it.

3. **Docker macvlan (private mode)** — inside the gateway Lima VM, each gateway container uses Docker macvlan (`-o macvlan_mode=private`) on the socket_vmnet NIC for its session's slot, sharing that segment's L2 with the external sandbox VM. (Empirically verified on macOS 26.4 / Lima 2.1.1 / QEMU 11.0.1: a macvlan child's distinct MAC traverses socket_vmnet to the sandbox VM.) The gateway container also carries a **second NAT-bridge NIC** inside the Lima VM for its own upstream egress (→ the Lima VM's slirp uplink → host).

**vmType `qemu`, never VZ — and never vzNAT or SLIRP.** macOS sandbox VMs *and* the gateway Lima instance use `vmType: "qemu"` with socket_vmnet, **not** Apple's VZ. socket_vmnet attaches to QEMU, not VZ (VZ's `VZFileHandleNetworkDeviceAttachment` is wire-incompatible with socket_vmnet's QEMU framing — [lima-vm/socket_vmnet#13](https://github.com/lima-vm/socket_vmnet/issues/13)). Two "the hypervisor NATs the guest straight to the internet" attachments must **never** be the VM's general-egress NIC, because each bypasses the gateway (PREROUTING DNAT has nothing to intercept; Envoy/mitmproxy see nothing): VZ's **vzNAT** (`VZNATNetworkDeviceAttachment`) and QEMU's user-mode **SLIRP** (`-netdev user`) used as a general-egress NIC. Each sandbox VM and the gateway VM attach socket_vmnet via a **per-VM** `networks: [{socket: <slot-socket-path>}]` entry as the gateway-facing NIC — not a named network in the global `networks.yaml` (Lima rejects `socket:` there).

**Management slirp NIC must be `restrict=on` (both platforms).** Lima's management NIC *is* a QEMU user-mode/slirp interface (`eth0`) and cannot be removed — Lima needs it for SSH/readiness. A hardware spike (2026-06-14) confirmed that NIC NATs straight to the internet, and that merely giving the gateway a lower-metric default route (what the Linux `qmp.rs` guest script does, `metric 50`) is **overridable by a root guest** (`ip route del default …` falls back to `eth0`). The management NIC must therefore be configured **`-netdev user,restrict=on`**, which makes libslirp drop all guest-initiated egress while preserving the host→guest SSH `hostfwd` (spike-verified: internet blocked even after the guest re-adds the route; SSH and the socket_vmnet gateway path unaffected). This is enforced at the QEMU/slirp layer, not by route metrics, and applies to **both macOS and Linux** — Linux implements it in the `lima.rs` qemu-wrapper (#65, commit `4b134ec`; gated by `SANDBOX_UNRESTRICTED_SLIRP_FOR_PROVISIONING` so base-image builds stay unrestricted), and macOS mirrors the same mechanism. **Scope:** `restrict=on` is for **sandbox** VMs only. The **gateway** Lima VM's `eth0` is the sanctioned egress *uplink* and must **never** be restricted — restricting it would sever all egress; it is bounded instead by a default-drop baseline egress nft policy on `eth0` (allow only the gateway containers' re-originated upstream flows + DNS). The wrapper-injection is gated on instance kind (sandbox VM), not platform alone.

**Session lifecycle on macOS:**

* **Session start:** Claim a vmnet slot from the pool. Create a gateway container with macvlan (private mode) on the gateway Lima VM's **pre-attached** NIC for that slot. Boot the sandbox VM on the same slot's segment. The two share a **/29** subnet — only they can communicate.
...
macOS host
├── sandboxd
├── socket_vmnet pool (N root daemons, each an isolated /29 segment)
│
├── Gateway Lima VM (one, sandboxd-managed: "sandboxd-gateway", qemu)
│   ├── NIC-0 on slot-0 → Gateway container session-A (macvlan, private)
│   ├── NIC-1 on slot-1 → Gateway container session-B (macvlan, private)
│   └── (all N NICs pre-attached at boot; containers come/go per session)
│       gateway containers egress via a NAT bridge → Lima VM eth0 → host
│
├── Sandbox VM session-A on slot-0 → routes to gateway-A
├── Sandbox VM session-B on slot-1 → routes to gateway-B
└── ...
```

The sandbox VM's default route points to its gateway's IP on the /29 subnet. Traffic arrives at the gateway container with the original destination intact — PREROUTING DNAT works exactly as on Linux. From the gateway container's perspective, the traffic flow is indistinguishable from the Linux TAP-on-bridge model. Both platforms use the same gateway container image — only the network attachment differs (Docker bridge on Linux, macvlan on a per-session socket_vmnet segment on macOS).

Each session is fully isolated at L2 — the socket_vmnet instance, gateway container, and sandbox VM form a private /29 network segment. No cross-session traffic is possible, the same as Linux's per-session bridges.

The shared gateway Lima VM introduces the only cross-session failure mode in the architecture. On Linux, gateway containers are independent — a single gateway crash affects only its session, and all other sessions continue unaffected. On macOS, the gateway Lima VM hosts all gateway containers, so if it crashes, every active session loses networking simultaneously. This is an inherent consequence of the macOS platform constraint (Docker requires a Linux VM), not a design flaw. sandboxd must monitor the gateway Lima VM's health, restart it on failure, and recreate gateway containers afterward; sessions will experience a networking interruption during recovery. All other failure modes remain session-scoped on both platforms.
...
* **Gateway egress.** The gateway container's own outbound connections — Envoy forwarding permitted requests, the DNS resolver querying upstream nameservers — leave via a NAT-masqueraded path, never over the VM-facing macvlan (which is `private`). On Linux, Docker applies MASQUERADE on the host's external interface for traffic leaving the session bridge; the gateway's outbound packets are SNAT'd to the host IP. On macOS, the gateway container has a second NIC on a NAT bridge inside the sandboxd-managed **gateway Lima VM**; its outbound traffic is masqueraded out through the gateway Lima VM's `eth0` (slirp/management) uplink to the host. No `--network=host` or special egress privileges are needed.

#### Subnet allocation

Each session gets its own per-session subnet, carved from a configurable base range chosen from an uncommon RFC 1918 slice to minimize conflicts with VPN and corporate networks. Operators can override the base range. The subnet size differs by platform, by participant count:

* **Linux: /28** (16 addresses; base default `10.209.0.0/24`). Docker bridge `.1`, gateway container `.2`, VM `.3`.
* **macOS: /29** (6 usable; base default `10.209.128.0/24`, distinct from Linux to avoid dual-boot/shared-config clashes). With static, network-identifier addressing the L3 participants are the gateway container (`B+3`) and the sandbox VM (`B+4`); the macvlan parent (`B+2`) is link-only and an isolated network-identifier segment has no host gateway (`B+1`). /29 is retained for headroom (a /30's 2 usable IPs are tight; final sizing is pending the isolation spike — spec task A).

The gateway's IP on the per-session subnet serves as:
...
These components form the layered enforcement pipeline described throughout this document. The gateway container runs with no `--privileged`, no host PID namespace, no host filesystem mounts beyond configuration volumes, and a read-only root filesystem with writable volumes for logs and runtime state. It is granted exactly **one** added capability — `CAP_NET_ADMIN` (`--cap-add NET_ADMIN`) — and nothing else. sandboxd injects the nftables ruleset by running `nft -f -` **inside** the container via `docker exec` (`GatewayManager::inject_nftables_ruleset`), so the container needs `CAP_NET_ADMIN` to manage rules in its own network namespace. (Route repointing is a separate operation: in lite mode `sandbox-route-helper` installs the sandbox's default route from *outside* the container's netns, which is why that helper — not the gateway container — carries the route-installing capabilities.)
...
* the VM's network topology is the primary constraint — one egress-capable NIC (default route → gateway) plus a `restrict=on` management NIC that cannot egress, so there is no alternate exit path even with root access inside the VM (the agent runs as root by default; non-root is an optional operator hardening setting)
