...
cleanup_tmpdir() {
    ...
    # Ensure cursor is visible — a repaint interrupted mid-hide leaves the
    # cursor hidden, which persists on the primary screen after rmcup.
    if [ -n "$UI_TTY" ]; then
...
ui_render_checklist() {
    ...
    # Show cursor after repaint completes.
    printf '\033[?25h' >"$UI_TTY" 2>/dev/null || true
