...
**Goal.** Three workspace changes under one umbrella, all delivered against the 2026-05-20 spec (which supersedes the narrower 2026-05-14 9p-securityModel spec): (1) make the 9p `securityModel` of `shared:` selectable per session — default `mapped-xattr`, opt-in `none` for real-symlink interop both directions; `passthrough` / `mapped-file` deliberately not exposed; (2) make the `shared:` guest path configurable with a breaking default of *preserving the host path* inside the guest (so build artefacts referring to the working directory survive a host↔guest round-trip) — pre-0.1.0 breaking-default change accepted; (3) add a new `local:` workspace mode — rsync-based host-snapshot at session create plus operator-driven `sandbox workspace push` / `pull` (`-f`/`-n` safety gate, `--safe-links`, `--no-gitignore`), no 9p or bind-mount surface, no live host writes. CLI grammar grows to `shared:<host>[:<guest>][:<model>]` and `local:<host>[:<guest>]`. M17-S1 ships the `shared:` parser + guest-path + securityModel + breaking default; M17-S2 ships `local:` end-to-end (parser, daemon-side rsync orchestration, push/pull CLI, both backends, integration + E2E); M17-S3 is the six-track review across S1+S2; M17-S4 is the spec-delivery verification. Terminal milestone for the 2026-05-20 spec.
**Status.** not_started · **Sessions.** 4 · **Details.** [milestones/M17.md](milestones/M17.md)
