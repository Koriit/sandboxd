...
**In scope (pointers into the spec):**
- Spec § "Component changes → mitmproxy": flip to regular (forward-proxy) mode — flags `--mode regular --listen-host 127.0.0.1 --listen-port 18080` in the gateway `entrypoint.sh`. Upstream target is read from the `CONNECT host:port` line Envoy sends; no `SO_ORIGINAL_DST` dependency in mitmproxy post-flip. The current `mitmproxy 12.2.3` supports this natively.
