...
- Subnet allocator:
  - Configurable base range (in-code default `10.209.0.0/24`; the M11-S9 dev-install template at `contrib/users.conf.example` overrides this to `10.209.0.0/20` for additional headroom — `users.conf` overrides the in-code default)
