...
# more permissive mode under a pre-exec race. See spec § 5.1.
UMask=0117
...
> and removed again in M16-S13: with `sandbox-guest` shipping at the
> FHS libexec path (atomically renamed by `sandbox update`), the
> container backend bind-mounts the libexec source directly and the
> daemon no longer maintains a state-dir copy. Spec 2 § 3.8.1 carries
> the amended bind-mount design.
...
1. Doctor runs C1; if it fails, C2-C13 (excluding C4, C9, and C10)
   fall through (they require a running, reachable daemon). They are
   reported as `SKIPPED`, not `FAILED`, with an `(requires daemon)`
   annotation. C4 (group membership) and C9 (route-helper caps) can
   still run — they don't depend on the daemon — and are evaluated.
   C10 (state-dir mode) needs read access to `/var/lib/sandbox/`; the
   parent dir is `0750` so an operator in `sandbox` group can stat it.
   If they're not in the group, doctor reports C10 as
   `SKIPPED (requires sandbox group membership)`.
...
C13 is a warning (informational): detected orphans do **not** contribute
to the exit-code-1 condition. Orphans are harmless resource leaks, not
active problems; operators should clean them up but nothing breaks if
they don't. C13 appears in the `--verbose` summary always; in default
mode only when orphans are present (surfacing them is actionable even
without `--verbose`).
...
### 7.6 · `sandboxd --version` output format

Spec 4 § 4.4.5's half-installed-state detection runs:

```sh
existing_ver=$(/usr/local/bin/sandboxd --version 2>/dev/null | awk '{print $2}')
```

The `awk '{print $2}'` expression depends on the output being exactly
two space-separated tokens with the semver in column 2. This format is
**pinned as part of the daemon's public interface**:

> `sandboxd --version` produces exactly one line: `sandboxd <semver>`
> followed by a newline — e.g., `sandboxd 1.0.0\n`.

The same pin applies to `sandbox --version` (the CLI binary):

> `sandbox --version` produces exactly one line: `sandbox <semver>\n`
> — e.g., `sandbox 1.0.0\n`.

Rules:

- One line, terminated by a single newline. No banner, no trailing
  blank line.
- Exactly two tokens separated by a single space: binary name then
  semver.
- The semver token is `env!("CARGO_PKG_VERSION")` — no architecture
  suffix, no build SHA suffix, no `v` prefix.
- Output goes to **stdout**; exit code `0`.

Implementation: clap derives `--version` automatically from the
`#[command(name = "...", version)]` attribute — `version` without
an explicit value picks up `CARGO_PKG_VERSION` and the binary name
from `name`. No custom handler needed. Implementers must confirm
the actual clap output matches this pattern before release.

Spec 4 § 4.4.5's `awk '{print $2}'` parse is correct by derivation
from this pin. If this format ever needs to change, Spec 4's parse
expression must be updated at the same time — the two are co-evolved.
...
| C10 state dir mode | **SKIPPED** — In dev mode the parent dir is operator-owned (created by `tokio::fs::create_dir_all` at the operator's umask, not by systemd's `StateDirectory`). The `sandbox` user and `sandbox:sandbox` ownership that C10 checks do not exist on dev boxes. Doctor skips C10 with annotation `(dev mode — no sandbox user/systemd StateDirectory)`. |
...
### 13.2 · KVM check from CLI's UID; `/diagnostics` filter boundary
...
C13). The endpoint carries **two distinct classes of data**:

**System-level diagnostics** — properties of the daemon process itself,
not scoped to any operator. Returned verbatim to every connected
operator:
...
**Per-operator scoped data** — filtered to the calling operator's
sessions only. The endpoint takes `Extension<OperatorIdentity>` (from
the `SO_PEERCRED` infrastructure of Spec 1 § 6 / Spec 2 § 4), and
threads `operator.name` into `SessionStore::list_sessions(caller)` to
scope these fields:
...
`guest_version_drift` entries contain only sessions owned by the
calling operator — the same `owner_username` filter Spec 2 § 2.2
applies on every other session endpoint. `substrate_orphans` is the
C13 cross-reference: substrate resources are enumerated host-wide
(from `limactl list` / `docker ps -a` / the filesystem), then
cross-referenced against the caller's session list; a resource
matching `sandbox-<id>` where `<id>` is not in the caller's session
list is reported as an orphan. An operator cannot infer another
operator's session IDs from the orphan list: they only see resources
that **they** cannot account for.

Security boundary note: the endpoint is partially unauthenticated
(system-level diagnostics are not sensitive — KVM readability and
image presence are host-global facts) and partially filtered
(per-session data filtered by caller). This is the correct split:
exposing system health to every operator in the `sandbox` group is
acceptable; exposing other operators' session IDs is not.

The `unresolvable peer-cred` close-on-failure policy (Spec 1 § 9.1 /
Spec 2 § 4) applies here as on every other endpoint: if
`SO_PEERCRED.uid` does not resolve, the connection is closed before
the handler runs.
