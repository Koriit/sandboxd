...
- **UDP DNS race for non-A queries.** AAAA and SVCB/HTTPS are stripped
  pre-emptively (`networking/coredns-plugin/handler.go:49-68`); they
  never reach the gating path. No change here.
...
| `qtype` | yes | `A` only in v1; the gate is not invoked for AAAA / SVCB / HTTPS (those are stripped pre-emptively) |
...
- **UDP DNS race for non-A queries.** AAAA / SVCB / HTTPS responses
  are stripped before reaching the gate; their integrity is unchanged.
