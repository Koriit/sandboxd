...
| P4.36 | Step 10 test-only `SANDBOX_INSTALL_SKIP_SIGSTORE=1` bypass (warn-level log; absent from operator-facing installer because docs-deploy sed-strips `BEGIN_TEST_ENV…END_TEST_ENV` spans before publishing to GitHub Pages) | (a) | `scripts/install.sh` `sigstore_verify()` source-tree body wrapped in `BEGIN_TEST_ENV` sentinels; `.github/workflows/docs.yml` "Stage install scripts" runs the strip and a `grep` self-check | `tests/install-e2e/test_install_air_gapped.py::test_install_air_gapped` (source-tree script in Lima) asserts `test-env-override` log token; sed-stripped output verified out-of-band against the documented regex `SANDBOX_(INSTALL\|UPDATE)_TEST_\|_DEBUG_COSIGN_STDERR\|_SKIP_SIGSTORE` |
