...
| # | Claim | Status | Code | Test |
|---|-------|--------|------|------|
| P3.15 | Spec § 3.6 picks option A (`include_bytes!` in `sandbox-core`); implementation chose option B (sibling-binary resolve + tempfile stage) instead — explicit deviation | (b) | `sandboxd/sandbox-core/src/guest.rs:89-130` `stage_embedded_guest_binary` reads `lima::guest_agent_path()` then writes to a tempfile (NOT `include_bytes!`); doc-comment at `:93-100` documents the deviation: "Spec 2 § 3.6 specifies `include_bytes!` for production builds (Spec 4 territory); for Spec 2's dev-mode-only scope, the daemon reads the sibling binary via the existing `guest_agent_path` resolver and stages it through `tempfile::NamedTempFile`" — i.e., explicitly deferred to Spec 4 with a documented seam | The deferral is in-source; no test required — Spec 2's dev-mode constraint accepts this. § 8 bullet 1 (Specs 4/5 deferral) is the formal scope-out anchor. Spec 4 will swap the implementation for `include_bytes!` |
...
| P3.19 | Container backend impl: docker stop (idempotent) → stage embedded bytes to host tempfile → `docker cp` into container → drop tempfile; does NOT call `docker start` (the orchestrator's next step does) | (a) | `sandboxd/sandbox-core/src/backend/container.rs:685-731` matches the spec sequence exactly (`docker stop -t 5` at `:693-704`, stage at `:706-715`, `docker cp` at `:719-724`, drop at `:728`) | `sandboxd/sandboxd/tests/integration_guest_refresh.rs:377` `integration_guest_refresh_container_backend` (verifies stop+cp; asserts in-container binary bytes match daemon-side bytes; checks idempotency with second call) |
| P3.20 | `docker cp` works on read-only containers via the storage driver | (a) | Confirmed by `integration_guest_refresh_container_backend` actually running against `--read-only` container (lite-mode default at `container.rs:482`) — successful refresh proves it | `sandboxd/sandboxd/tests/integration_guest_refresh.rs:377` exercises against the configured container |
...
| P7.23 | `integration_guest_refresh_container_backend` | (a) | `sandbox-core/src/backend/container.rs:685-731` | `sandboxd/sandboxd/tests/integration_guest_refresh.rs:377` `integration_guest_refresh_container_backend` |
...
| P8.4 | § 9.4 — `docker cp` on `--read-only` container works through the storage driver; supported surface is overlay2 on regular dockerd | (a) | `sandbox-core/src/backend/container.rs:685-731` `docker cp` against `--read-only` container | `sandboxd/sandboxd/tests/integration_guest_refresh.rs:377` `integration_guest_refresh_container_backend` runs against the project's standard `--read-only` lite image |
