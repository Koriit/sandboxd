...
Post-M11-S9: **0 proposed new todos**. M11-S9 closed three
converging threads — route-helper env-var feature gate
(privilege-escalation closure), daemon resolver collapse to
two-step lookup, and TCP-test determinism — alongside the install-
path move to FHS § 4.7 libexec, the new `make setup-dev-env`
...
| LM2.10 | Trait surface stateless over `RuntimeHandle`; one instance per `BackendKind` shared across all sessions, with `ContainerRuntime` carrying per-session network state on a daemon-populated side map (M11-S3 Phase 3A compromise) | (a)    | `sandboxd/sandboxd/src/main.rs:544-580` `runtimes: HashMap<BackendKind, Arc<dyn SessionRuntime>>` (one entry per kind); `sandboxd/sandbox-core/src/backend/container.rs:245` `sessions: Mutex<HashMap<SessionId, ContainerNetwork>>` populated by `ContainerRuntime::register_session` before `SessionRuntime::create` | shared by trait shape; `sandboxd/sandboxd/tests/integration_backends_endpoint.rs:32` `integration_backends_endpoint_lists_registered_backends_in_stable_order` confirms one entry per kind |
...
## LM13 — Route-helper auth hardening, dev-environment setup, test stability (LM13.1 – LM13.6)

M11-S9 closed three converging threads: (1) the route helper's
`SANDBOX_USERS_CONF` env-var read was unconditional and — because the
helper runs with `cap_sys_admin+ep` — represented a local privilege-
escalation vector; (2) the daemon's route-helper resolver had four
implicit candidate sources with silent fallbacks that were either
invisible (`$PATH` outside the operator's awareness) or mis-targeted
(sibling-of-daemon picks an un-cap'd `target/debug/` build); and (3)
the `tcp_respects_concurrency_cap` test was non-deterministic, with
a 50-attempt retry loop that tolerated `ECONNREFUSED` and `ECONNRESET`
indistinguishably and could double-count over-cap RST'd connections.
This cluster maps each landed change to its code+test locators.

| #      | Claim                                                                                                                                                                                                              | Status | Code                                                                                                                                                                                                                                                                                                                                                                                                                                                | Test                                                                                                                                                                                                                                                                                                                                                                |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LM13.1 | `SANDBOX_USERS_CONF` env-var consultation in the route helper is feature-gated behind `test-env-override` on the `sandbox-core` crate; default builds ignore the env var unconditionally; `sandbox-route-helper`'s `test-env-override` feature forwards | (a)    | `sandboxd/sandbox-core/Cargo.toml` `[features] test-env-override = []`; `sandboxd/sandbox-core/src/users_conf.rs::route_helper_users_conf_path` (`#[cfg(feature = "test-env-override")]` gate around the env read); `sandboxd/sandbox-route-helper/Cargo.toml` `[features] test-env-override = ["sandbox-core/test-env-override"]`; route-helper `main.rs` switched to `load_users_config_route_helper`                                              | `sandboxd/sandbox-core/src/users_conf.rs::tests::route_helper_users_conf_path_ignores_env_in_default_build` (negative arm) and `:route_helper_users_conf_path_honors_env_with_test_env_override_feature` (positive arm); `sandboxd/sandbox-route-helper/src/main.rs::tests::route_helper_path_resolution_ignores_env_in_default_build` + `_honors_env_with_test_env_override_feature` (same split, asserts the privilege boundary at the route-helper crate, where the `cap_sys_admin+ep` xattr is applied) |
| LM13.2 | Defensive ownership/mode check on the canonical `/etc/sandboxd/users.conf` path: refuses non-root-owned and group/world-writable files at config-load time; tempfile paths bypass the check                       | (a)    | `sandboxd/sandbox-core/src/users_conf.rs::validate_canonical_users_conf_security` and the parameterized inner `validate_users_conf_security_against`; new `UsersConfigError::InsecureFile` variant with operator-actionable Display message pointing at the install docs                                                                                                                                                                            | `sandboxd/sandbox-core/src/users_conf.rs::tests::defensive_check_refuses_non_root_owned_canonical_file`; `:defensive_check_refuses_world_writable_canonical_file`; `:defensive_check_skips_when_path_is_not_canonical` (proves tempfile-based tests are unaffected); `:defensive_check_skips_when_canonical_file_does_not_exist` (defers to `FileNotFound` downstream)                                                                                              |
| LM13.3 | Production install path moved from `/usr/local/bin/sandbox-route-helper` to `/usr/local/libexec/sandboxd/sandbox-route-helper` per FHS § 4.7; test install lives at the parallel `/usr/local/libexec/sandboxd-test/sandbox-route-helper` so test installs do not clobber production binaries | (a)    | `sandboxd/sandboxd/src/main.rs::ROUTE_HELPER_INSTALL_PATH` constant updated; `Makefile::ROUTE_HELPER_PROD_PATH` and `ROUTE_HELPER_TEST_PATH` carry the canonical paths; spec § "Install-time setup" amended; `docs/start/installation.md` § "sandbox-route-helper" updated                                                                                                                                                                            | install paths consumed by `Makefile::install-route-helper-prod-cap` and `:install-route-helper-test-cap` targets and verified at runtime by the resolver's existence/cap check                                                                                                                                                                                       |
| LM13.4 | Daemon route-helper resolver collapsed from 4 candidate sources to 2: (0) `$SANDBOX_ROUTE_HELPER_PATH` (fail-closed if set but missing/un-cap'd), (1) `/usr/local/libexec/sandboxd/sandbox-route-helper`. Sibling-of-daemon and `$PATH` lookups dropped; cap-xattr check stays. Failure error names both candidate paths and the make target | (a)    | `sandboxd/sandboxd/src/main.rs::resolve_route_helper_path` (env-var read + canonical path) and `:resolve_route_helper_path_from` (pure inner, parameterized over `is_usable`); module-level comment rewritten to document the two-step shape; old `default_route_helper_candidates` helper removed                                                                                                                                                  | `sandboxd/sandboxd/src/main.rs::tests::resolve_route_helper_path_from_uses_env_override_when_set_and_usable`; `:resolve_route_helper_path_from_errors_when_env_override_set_but_unusable` (asserts fail-closed semantics); `:resolve_route_helper_path_from_uses_canonical_when_env_unset_and_usable`; `:resolve_route_helper_path_from_errors_when_env_unset_and_canonical_unusable` (error names make target + cap requirement)                            |
| LM13.5 | `make setup-dev-env` orchestrator + 5 sub-targets (`install-route-helper-prod-cap`, `install-route-helper-test-cap`, `setup-bridge-conf`, `setup-users-conf`, `setup-bridge-helper-setuid`); each prints `[sudo] <exact change>` before invoking sudo; idempotent (re-run prints `✓ already configured` and invokes no sudo); `make test-integration` and `make test-e2e-{container,matrix}` depend on the appropriate install targets | (a)    | `Makefile::setup-dev-env` and the five sub-target rules; stamp files under `sandboxd/target/.dev-env-stamps/` carry route-helper install state; `contrib/users.conf.example` provides the `$USER`-substituted template; `setup-bridge-conf` refuses to silently mutate an existing `/etc/qemu/bridge.conf` with conflicting content                                                                                                                | idempotency verified by re-running `make setup-dev-env` on a configured host; this is operator-facing make-target plumbing, not a Rust test surface                                                                                                                                                                                                                  |
| LM13.6 | TCP listener backlog made explicit on `sandbox-deny-logger`'s `tcp::bind`; `tcp_respects_concurrency_cap` test rewritten to be deterministic (one `connect()` per thread, no retries, single-shot ECONNRESET → server-rejected accounting); conservation invariant `deny + dropped_total == BURST` holds without retry-loop tolerance | (a)    | `sandboxd/sandbox-deny-logger/Cargo.toml` adds `socket2` direct dep; `sandboxd/sandbox-deny-logger/src/tcp.rs::bind` accepts `backlog: i32` and uses `socket2::Socket::listen(backlog)`; `sandboxd/sandbox-deny-logger/src/main.rs` passes `conn_cap * 4` clamped to i32 as the production backlog                                                                                                                                                  | `sandboxd/sandbox-deny-logger/src/tcp.rs::tests::tcp_respects_concurrency_cap` (deterministic shape with `BACKLOG = BURST * 4`); the existing `tcp_emits_one_deny_event_and_rst_closes` updated to pass an explicit backlog of 128                                                                                                                                  |
